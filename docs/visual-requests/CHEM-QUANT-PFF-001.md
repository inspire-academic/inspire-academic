...
**STATUS: APPROVED — CANONICAL.** This request has run end to end:
generated by ChatGPT/OpenAI, approved by human scientific/pedagogical/
visual review, integrated by Claude without redrawing or reconstructing
any content inside the figure, and passed full live QA on first
integration. See the "FINAL APPROVAL" section at the end of this file,
and `docs/production/INSPIRE-VISUAL-ASSET-PIPELINE-PROPOSAL.md` v0.3
for the closing evidence this POC contributed to the four-mode
representation router — this was the first request authored, generated,
and canonicalised end to end under **Mode C (Premium Final Figure)**.
...
Steps 1–5 above (verify, convert/optimise, integrate, test, preserve
fallback) were all performed and all passed. Evidence:

- **Canonical asset**: `assets/images/chemistry/diagrams/quantitative/CHEM-QUANT-PFF-001.webp`
  — 960×640, 66.0KB (source PNG was 1536×1024, 2.77MB; converted and
  compressed, not regenerated).
- **Integration**: `teaching-lessons/chemistry/quantitative-chemistry-relative-formula-mass-moles.html`,
  Core Lesson, immediately after Stage 2 — exactly the placement
  specified above.
- **Redraw/reconstruction**: none. Every label, value, arrow, and
  annotation visible in the figure is exactly what ChatGPT/OpenAI
  generated; Claude added only the surrounding `<figure>`/`<figcaption>`
  wrapper, alt text, and one CSS rule for responsive sizing.
- **QA**: dark theme PASS, light theme PASS, overflow PASS, console
  PASS, network (200 on the asset request) PASS, accessibility layer
  PASS, performance budget PASS (66.0KB — under the 80KB ceiling; the
  only raster image request on this lesson page, so also the page's
  entire image weight, well inside CLAUDE.md's <100KB budget).
- **Corrective visual pass after integration**: none required — passed
  on first integration attempt, unlike POC #1 (`PHY-FOR-HYB-001`),
  which needed three.
- **Integration commit**: `b0a9803`.

**Editorial note (non-blocking, does not affect this approval)**: the
figure carries its own internal title ("The Mole: A Counting Unit")
immediately adjacent to the lesson's own visible "Stage 2" heading. This
is not a scientific, notational, or accessibility defect and this asset
is approved as-is and must not be modified because of it — it is
recorded purely as forward-looking editorial guidance for future
Premium Final Figure requests. See
`docs/production/INSPIRE-VISUAL-ASSET-PIPELINE-PROPOSAL.md` v0.3, "v0.3
UPDATE" section, "Editorial note — internal title hierarchy," for the
full guidance.

This is the second manual visual POC to reach canonical status
(alongside `PHY-FOR-HYB-001`, Mode D) and the first to do so without
requiring any corrective pass. See the pipeline proposal's v0.3 update
for the combined evidence and architecture conclusion both POCs
together establish.
