...
Method A: 40% of 33,600 kJ is wasted, so 0.40 × 33,600 = 13,440 kJ wasted, and 33,600 − 13,440 = 20,160 kJ is usefully stored. Efficiency = 20,160 ÷ 33,600 × 100 = 60%.
Method B: 25% of 490 kJ is wasted, so 0.25 × 490 = 122.5 kJ wasted, and 490 − 122.5 = 367.5 kJ is usefully stored. Efficiency = 367.5 ÷ 490 × 100 = 75%.

Step 2: compare using those numbers. Method A stores far more energy per 100 kg (33,600 kJ vs 490 kJ), since heating water is a much denser way to store energy, but a smaller fraction of it is usefully recovered (60% vs 75%), because hot water loses energy to its surroundings unless well insulated. Method B is more efficient, but Method A can be installed anywhere a hot water tank fits, while Method B needs a very specific site (high ground, a reservoir). The real trade-off: Method A wins on flexibility of location, Method B wins on efficiency and lower losses.
...
$q$A spark is really just current jumping through the air once the electric field is strong enough to ionise the air and let charge flow. Anything that makes it easier for current to flow through the air (lowering the air's electrical resistance) lets a spark jump a larger gap. The other three options all work against a spark forming: decreasing the charge, the field strength, or the potential difference all make it harder (not easier) for a spark to jump, so a bigger gap needs more of those, not less.$q$,
'AO3', 17
FROM past_papers pp JOIN subjects s ON s.id = pp.subject_id
WHERE s.name='Physics' AND pp.exam_board='AQA' AND pp.tier='Higher' AND pp.year=2024 AND pp.series='June' AND pp.paper_number=1;
...
With such a tiny mass of air (2.60×10⁻⁸ kg, a fraction of a milligram) and a specific heat capacity that isn't huge, even a small energy transfer (0.0130 J) produces a large temperature rise. That's exactly the physics of why a fire piston works: rapidly compressing a small amount of air can heat it enough to ignite something flammable.$q$,
'AO2', 33
FROM past_papers pp JOIN subjects s ON s.id = pp.subject_id
WHERE s.name='Physics' AND pp.exam_board='AQA' AND pp.tier='Higher' AND pp.year=2024 AND pp.series='June' AND pp.paper_number=1;
